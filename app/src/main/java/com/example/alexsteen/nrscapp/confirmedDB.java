...
public class confirmedDB extends SQLiteOpenHelper{

    private static final String DATABASE_NAME = "confirmed.db";
    ...
    public confirmedDB(Context context) {
        super(context, DATABASE_NAME, null, 1);
    }

    @Override
    public void onCreate(SQLiteDatabase db) {
        String query = "CREATE TABLE " + TABLE_CONFIRMED + " ("
    ...
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        ...
        db.execSQL("DROP_TABLE_IF_EXISTS " + TABLE_CONFIRMED);
        onCreate(db);
    }

    public LinkedList<String> listOfInterestedVendors(String venueName) {
        SQLiteDatabase db = getReadableDatabase();
        Cursor c = db.rawQuery("SELECT * FROM " + TABLE_CONFIRMED + " WHERE " + COLUMN_VENUE_NAME + " = " + venueName +";", null);
        c.moveToFirst();
