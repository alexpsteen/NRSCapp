...
public class eventPlannerDB extends SQLiteOpenHelper{

    private static final String DATABASE_NAME = "eventPlanner.db";
    private static final String TABLE_EVENT_PLANNERS = "eventPlanners";
    ...
    public eventPlannerDB(Context context) {
        super(context, DATABASE_NAME, null, 1);
    }

    @Override
    public void onCreate(SQLiteDatabase db) {
        String query = "CREATE TABLE " + TABLE_EVENT_PLANNERS + " ("
    ...
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        ...
        db.execSQL("DROP_TABLE_IF_EXISTS " + TABLE_EVENT_PLANNERS);
        onCreate(db);
    }
    ...
    public void addUsers(userObject user){
        ...
        db.insert(TABLE_EVENT_PLANNERS, null, values);
        db.close();
    }

    public boolean authenticateUser(String username, String password) {
        SQLiteDatabase db = getReadableDatabase();
        Cursor c = db.rawQuery("SELECT * FROM " + TABLE_EVENT_PLANNERS + " WHERE " + COLUMN_USERNAME + " = '" + username + "';", null);
        c.moveToFirst();
        String realPassword;
        if (!c.isBeforeFirst()) {
