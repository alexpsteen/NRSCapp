...
function handleEventsGET (httpEvent, context) {
  ...
  if(taskType === "customer") {
    ...
    const inserts = [httpEvent.requestContext.identity.cognitoIdentityId];
    q = mysql.format(q, inserts);
  } else if(taskType === "vendor") {
    q = 'SELECT DISTINCT e.* FROM event e, user u, vendor v, feature f, recommend r WHERE v.user_id = u.user_id AND u.authentication_id = ? AND v.vendor_id = r.vendor_id AND r.feature_id = f.feature_id AND f.event_id = e.event_id AND r.confirm = 1;';
    const inserts = [httpEvent.requestContext.identity.cognitoIdentityId];
    q = mysql.format(q, inserts);
